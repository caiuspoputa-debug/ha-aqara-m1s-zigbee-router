...
- Periodic receiver resync is disabled. **Adaptive sync remains disabled**, so no per-member micro-resampling/rate correction is applied.
- `aqara_m1s_zigbee_router.resync_media_group` is available for manual realignment. It performs one shared group restart with a 20 s cooldown and refuses to leave only one eligible hub playing.
